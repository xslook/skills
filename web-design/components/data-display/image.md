...
> Identifier: `image`  
> Definition: Image display component with loading placeholders, fallback images, and preview modals.
...
### Core Purpose
Image display component with loading placeholders, fallback images, and preview modals.

### When to Use
Use for photo galleries, proof receipts, document attachments, and item thumbnails.

### When NOT to Use
Do not use for small semantic icons; use Icon instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Image Structural Hierarchy:
├── Root: Image wrapper.
├── Img: HTML image element.
├── Mask: Hover overlay with zoom icon.
├── PreviewModal: Fullscreen preview dialog with toolbar.
```
...
- **Thumbnail with click preview**: Thumbnail with click preview
- **Multi-image preview gallery with zoom/rotate controls**: Multi-image preview gallery with zoom/rotate controls
- **Fallback image on load failure**: Fallback image on load failure
...
- **ARIA Role**: `role="img"`
- **Key Attributes**: `alt="Image description"` required
- **Keyboard Navigation**: Enter opens preview dialog; Left/Right arrows navigate images; Escape closes.
