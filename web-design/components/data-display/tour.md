...
> Identifier: `tour`  
> Definition: Guided multi-step walkthrough modal highlighting specific interface elements.
...
### Core Purpose
Guided multi-step walkthrough modal highlighting specific interface elements.

### When to Use
Use for onboarding new users or introducing major feature redesigns.

### When NOT to Use
Do not use for everyday routine notifications or warning messages. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Tour Structural Hierarchy:
├── Mask: Fullscreen dark backdrop with cutout over target element.
├── Card: Floating guidance card.
├── Footer: Step counter with Next/Finish buttons.
```
...
- **Targeted highlight with cutout backdrop**: Targeted highlight with cutout backdrop
- **Standalone centered modal card**: Standalone centered modal card
...
- **Key Attributes**: `aria-modal="true"`, `aria-label="Onboarding tour"`
- **Keyboard Navigation**: Focus trapped in tour modal; Enter moves to next step; Escape cancels tour.
