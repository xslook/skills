...
> Identifier: `table`  
> Definition: Multi-column structured data grid with sorting, filtering, row selection, and pagination.
...
### Core Purpose
Multi-column structured data grid with sorting, filtering, row selection, and pagination.

### When to Use
Use for structured, multi-attribute datasets that require scanning, comparison, and batch operations.

### When NOT to Use
Do not use for unstructured media or text narratives; use List or Card grid instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Table Structural Hierarchy:
├── Root: Table container.
├── Header: Column titles with sorting and filtering icons.
├── Body: Data rows with selection checkboxes and cells.
├── Pagination: Attached footer pagination bar.
```
...
- **Bordered / Borderless**: Bordered / Borderless
- **Zebra striping**: Zebra striping
- **Fixed columns / Fixed header**: Fixed columns / Fixed header
- **Expandable sub-rows**: Expandable sub-rows
...
- **ARIA Role**: `role="table"`
- **Key Attributes**: Header `role="columnheader"`, Row `role="row"`, `aria-sort="ascending|descending"`
- **Keyboard Navigation**: Sortable headers support Enter to sort; Arrow keys navigate cells when grid navigation enabled.
