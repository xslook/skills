...
> Identifier: `tree`  
> Definition: Hierarchical tree view with expandable, selectable, and checkable nodes.
...
### Core Purpose
Hierarchical tree view with expandable, selectable, and checkable nodes.

### When to Use
Use to display directory structures, departmental hierarchies, and nested categories.

### When NOT to Use
Do not use for linear flat lists; use List or Table instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
├── Root: Tree container.
├── Node: Row item.
├── Switcher: Expand/collapse chevron.
├── Checkbox: Optional selection box.
...
- **Basic directory tree**: Basic directory tree
- **Checkable tree with cascaded selections**: Checkable tree with cascaded selections
- **Draggable tree for reordering**: Draggable tree for reordering
...
- **ARIA Role**: `role="tree"`
- **Key Attributes**: Node `role="treeitem"`, `aria-expanded="true|false"`, `aria-selected="true"`
- **Keyboard Navigation**: Up/Down arrows traverse nodes; Right arrow expands; Left collapses; Space/Enter selects or checks.
