...
> Identifier: `empty`  
> Definition: Empty state placeholder signaling that no data exists.
...
### Core Purpose
Empty state placeholder signaling that no data exists.

### When to Use
Use in empty tables, blank search results, or zero-notification inboxes.

### When NOT to Use
Do not use when content is currently loading; use Skeleton or Spin instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Empty Structural Hierarchy:
├── Root: Centered layout container.
├── Image: Conceptual SVG illustration.
├── Description: Explanatory text.
├── Action: Call-to-action button slot.
```
...
- **Default illustration**: Default illustration
- **Simple minimal graphic**: Simple minimal graphic
...
- **Key Attributes**: `aria-label="No data available"`
- **Keyboard Navigation**: Action button inside empty state receives standard Tab focus.
