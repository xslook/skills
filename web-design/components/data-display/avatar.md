...
> Identifier: `avatar`  
> Definition: Visual representation of a user profile with image, text, or icon.
...
### Core Purpose
Visual representation of a user profile with image, text, or icon.

### When to Use
Use in navigation headers, comment threads, member lists, and user profile cards.

### When NOT to Use
Do not use as a standalone primary action button. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Avatar Structural Hierarchy:
├── Root: Circular or rounded square container.
├── Image: User photo.
├── Text: User initials with auto font-scaling.
```
...
- **Circle**: Circle
- **Square**: Square
- **Image / Icon / Text fallback**: Image / Icon / Text fallback
- **Avatar.Group**: Avatar.Group (overlapping group with `+N` badge)
...
- **ARIA Role**: `role="img"`
- **Key Attributes**: `aria-label="User avatar description"`
- **Keyboard Navigation**: Non-interactive; when wrapped in button, supports Tab and Enter.
