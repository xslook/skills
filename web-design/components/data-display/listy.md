...
> Identifier: `listy`  
> Definition: High-performance virtualized list for rendering tens of thousands of rows smoothly.
...
### Core Purpose
High-performance virtualized list for rendering tens of thousands of rows smoothly.

### When to Use
Use for massive real-time event streams, log viewports, or infinite data feeds where standard DOM nodes cause lag.

### When NOT to Use
Do not use for small lists with under 100 items; standard List has simpler markup. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Listy Structural Hierarchy:
├── Root: Scroll container with scrollbar.
├── Viewport: Rendered slice of visible rows based on scroll offset.
├── Item: Virtualized item row.
```
...
- **Fixed item height**: Fixed item height
...
- **ARIA Role**: `role="list"`
- **Key Attributes**: `aria-rowcount`, `aria-rowindex`
- **Keyboard Navigation**: Virtual scroll declares total count and current index to assistive tech.
