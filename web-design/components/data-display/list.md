...
> Identifier: `list`  
> Definition: Vertical collection of related content items.
...
### Core Purpose
Vertical collection of related content items.

### When to Use
Use for activity feeds, message streams, search result snippets, and task lists.

### When NOT to Use
Do not use when users need multi-column sorting and filtering; use Table instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
List Structural Hierarchy:
├── Root: List container.
├── Item: Single row with Meta slot (avatar, title, description) and Actions slot.
```
...
- **Simple borderless list**: Simple borderless list
- **Bordered list with header/footer**: Bordered list with header/footer
- **Item with avatar, title, description, and actions**: Item with avatar, title, description, and actions
...
- **ARIA Role**: `role="list"`
- **Key Attributes**: Standard list semantics
- **Keyboard Navigation**: Items use `role="listitem"`. Interactive links receive Tab focus.
