...
> Identifier: `tag`  
> Definition: Categorical label or status marker.
...
### Core Purpose
Categorical label or status marker.

### When to Use
Use for categorizing items, marking entity status (Active, Inactive, Pending), or keyword tagging.

### When NOT to Use
Do not use as an action button to trigger workflows; use Button instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Tag Structural Hierarchy:
├── Root: Rounded tag capsule.
├── Text: Label text.
├── CloseIcon: Optional remove cross icon.
```
...
- **Preset status tags**: Preset status tags (success, processing, error, warning)
- **Colored palette tags**: Colored palette tags
- **Closable tag**: Closable tag
- **Checkable tag**: Checkable tag (toggleable)
...
- **ARIA Role**: `role="status"` / `role="button"`
- **Key Attributes**: `aria-label="Tag name"`
- **Keyboard Navigation**: Close cross button receives Tab focus and deletes on Enter. Checkable tag toggles on Space.
