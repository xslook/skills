...
> Identifier: `qr-code`  
> Definition: Generates QR codes with support for canvas/SVG rendering and embedded center logos.
...
### Core Purpose
Generates QR codes with support for canvas/SVG rendering and embedded center logos.

### When to Use
Use for mobile authentication, payment gateways, and device pairing.

### When NOT to Use
Do not use for standard web-to-web navigation links; use regular buttons or text links. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
QRCode Structural Hierarchy:
├── Root: QR container with quiet border zone.
├── Canvas: Rendered QR matrix.
├── Overlay: Refresh icon overlay when expired.
```
...
- **With Center Logo**: With Center Logo
- **Expired / Refresh overlay state**: Expired / Refresh overlay state
...
- **ARIA Role**: `role="img"`
- **Key Attributes**: `aria-label="QR Code with content..."`
- **Keyboard Navigation**: Refresh button receives Tab focus and triggers re-generation on Enter.
