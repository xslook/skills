...
> Identifier: `collapse`  
> Definition: Expandable accordion panels for progressive information disclosure.
...
### Core Purpose
Expandable accordion panels for progressive information disclosure.

### When to Use
Use for FAQs, secondary configuration groups, or detail lists where users only need one section at a time.

### When NOT to Use
Do not hide primary required inputs inside collapsed accordions. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Collapse Structural Hierarchy:
├── Root: Panel stack container.
├── Header: Clickable row with expand chevron and title.
├── Content: Expandable body with height transition.
```
...
- **Standard accordion**: Standard accordion (single active panel)
- **Multiple expanded panels**: Multiple expanded panels
- **Borderless style**: Borderless style
...
- **Key Attributes**: Header `role="button"`, `aria-expanded="true|false"`
- **Keyboard Navigation**: Tab focuses header row; Enter or Space toggles expand/collapse.
