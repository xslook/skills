...
> Category: **Navigation**  
> Identifier: `menu`  
> Definition: Structured navigation menu supporting inline, horizontal, and vertical modes.
...
### Core Purpose
Structured navigation menu supporting inline, horizontal, and vertical modes.

### When to Use
Use as primary application navigation in top headers or sidebars.

### When NOT to Use
Do not use as a small contextual action list; use Dropdown instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Menu Structural Hierarchy:
├── Root: Menu container.
├── Item: Single navigation link.
├── SubMenu: Expandable nested branch.
├── ItemGroup: Titled category group.
```
...
- **Inline**: Inline (collapsible sidebar tree)
- **Vertical**: Vertical (flyout submenus)
...
- **ARIA Role**: `role="menubar"` or `role="menu"`
- **Key Attributes**: Item `role="menuitem"`, `aria-selected="true|false"`
- **Keyboard Navigation**: Arrow keys navigate items; Enter activates item or expands submenu; Escape closes submenu.
