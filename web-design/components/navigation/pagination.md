...
> Definition: Divides large datasets into discrete pages.
...
### Core Purpose
Divides large datasets into discrete pages.

### When to Use
Use when data records exceed 20 items to reduce load time and maintain scanning focus.

### When NOT to Use
Do not use when continuous infinite feed scrolling is required. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Pagination Structural Hierarchy:
├── Root: Horizontal flex list.
├── PrevBtn / NextBtn: Directional arrow buttons.
├── PageItem: Individual page number button.
├── QuickJumper: Direct page number input field.
```
...
- **Basic**: Basic (page numbers only)
- **Complete**: Complete (with total count, pageSize changer, and quick jumper)
- **Simple**: Simple (mini previous/next)
...
- **Key Attributes**: `aria-label="Pagination"`, `aria-current="page"` on active item
- **Keyboard Navigation**: Tab navigates page buttons; Enter changes page; Input accepts direct numbers.
