...
> Definition: Hierarchical navigation trail indicating page depth.
...
### Core Purpose
Hierarchical navigation trail indicating page depth.

### When to Use
Use for hierarchical systems with 2 or more nested layers to let users navigate upward.

### When NOT to Use
Do not use on flat 1-level dashboards with no parent routes. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
├── Root: Ordered list container.
├── Item: Link to ancestor page.
├── Separator: Divider glyph (default `/`).
```
...
- **Basic**: Basic (slash separated)
- **With Icons**: With Icons
- **With Dropdown Menus for intermediate steps**: With Dropdown Menus for intermediate steps
...
- **Key Attributes**: `aria-label="Breadcrumb"`, `aria-current="page"` on current item
- **Keyboard Navigation**: Tab navigates through links; Enter navigates.
