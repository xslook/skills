...
> Category: **Navigation**  
> Identifier: `steps`  
> Definition: Navigational progress bar guiding users through multi-stage workflows.
...
### Core Purpose
Navigational progress bar guiding users through multi-stage workflows.

### When to Use
Use for sequential processes (e.g., checkout, account setup, multi-step approvals).

### When NOT to Use
Do not use for independent, non-linear navigation. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Steps Structural Hierarchy:
├── Root: Progress bar container.
├── Item: Individual step node with icon, title, description, and connector line.
```
...
- **Horizontal**: Horizontal (standard wizard)
- **Vertical**: Vertical (tall panels)
- **Dot**: Dot (minimal milestone indicator)
...
- **Key Attributes**: `aria-label="Workflow steps"`, `aria-current="step"`
- **Keyboard Navigation**: Tab navigates through clickable steps; Enter activates step.
