...
> Category: **Navigation**  
> Identifier: `affix`  
> Definition: Pins elements to top or bottom of viewport during scrolling.
...
### Core Purpose
Pins elements to top or bottom of viewport during scrolling.

### When to Use
Use for sticky table headers, bottom submission bars in long forms, or floating navigation.

### When NOT to Use
Do not affix large containers that cover significant portions of the viewport. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Affix Structural Hierarchy:
├── Root: Placeholder preserving scroll height.
├── FixedNode: Position-fixed container when scroll threshold triggers.
```
...
- **Top Affix**: Top Affix (fixed to top offset)
- **Bottom Affix**: Bottom Affix (fixed to bottom offset)
