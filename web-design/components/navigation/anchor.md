...
> Category: **Navigation**  
> Identifier: `anchor`  
> Definition: Hyperlink navigation tree that tracks scroll position.
...
### Core Purpose
Hyperlink navigation tree that tracks scroll position.

### When to Use
Use in long technical documents, API references, or multi-section audit pages for quick navigation.

### When NOT to Use
Do not use for cross-page routing; use Menu instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Anchor Structural Hierarchy:
├── Root: Anchor container.
├── Item: Link item row.
├── Indicator: Animated active indicator line or dot.
```
...
- **Vertical Line**: Vertical Line (classic slider bar)
- **Dot Indicator**: Dot Indicator
- **Static vs. Affixed**: Static vs. Affixed
...
- **Key Attributes**: `aria-label="Anchor navigation"`
- **Keyboard Navigation**: Tab navigates between anchor links; Enter scrolls to target.
