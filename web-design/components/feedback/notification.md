...
> Definition: Corner notification card with title, description, and action links.
...
### Core Purpose
Corner notification card with title, description, and action links.

### When to Use
Use for asynchronous system broadcasts, background job completion notices, or multi-line alerts.

### When NOT to Use
Do not use for immediate inline feedback on simple user actions; use Message instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Notification Structural Hierarchy:
├── Root: Fixed corner stack container.
├── Card: Elevated notification card.
├── Icon: Status icon.
├── Title: Headline.
├── Desc: Multiline text.
├── CloseBtn: Dismiss icon.
```
...
- **TopLeft / BottomRight / BottomLeft**: TopLeft / BottomRight / BottomLeft
- **Status types**: Status types (Success, Info, Warning, Error)
...
- **Key Attributes**: `aria-live="polite"`
- **Keyboard Navigation**: Announced by screen reader; Close button accepts Tab and Enter.
