...
> Category: **Feedback**  
> Identifier: `skeleton`  
> Definition: Placeholder wireframe animation during content loading.
...
### Core Purpose
Placeholder wireframe animation during content loading.

### When to Use
Use while initial page content or cards are loading to reduce perceived waiting time.

### When NOT to Use
Do not use during fast interactions (< 200ms) to prevent visual flickering. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Skeleton Structural Hierarchy:
├── Root: Wireframe container.
├── Avatar: Pulsing circle.
├── Title: Pulsing bar.
├── Paragraph: Multi-line pulsing text bars.
```
...
- **Standard text paragraph wireframe**: Standard text paragraph wireframe
- **Avatar + Title + Paragraph**: Avatar + Title + Paragraph
- **Button / Input / Image shape skeletons**: Button / Input / Image shape skeletons
...
- **Key Attributes**: `aria-busy="true"`, `aria-label="Loading content"`
- **Keyboard Navigation**: Announces loading state during async data retrieval.
