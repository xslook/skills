...
> Category: **Feedback**  
> Identifier: `alert`  
> Definition: In-page banner warning or notice anchored within layout.
...
### Core Purpose
In-page banner warning or notice anchored within layout.

### When to Use
Use for persistent page-level or form-level warnings, system announcements, or critical prerequisite notices.

### When NOT to Use
Do not use for transient feedback after clicking a button; use Message instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Alert Structural Hierarchy:
├── Root: Tinted banner box.
...
- **Error**: Error (red)
- **Banner mode**: Banner mode (edge-to-edge)
...
- **Key Attributes**: `aria-live="assertive"` for errors, `aria-live="polite"` for notices
- **Keyboard Navigation**: Announced immediately by screen reader; Close button accepts Tab and Enter.
