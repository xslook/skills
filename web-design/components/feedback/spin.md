...
> Category: **Feedback**  
> Identifier: `spin`  
> Definition: Rotating circular indicator for asynchronous loading.
...
### Core Purpose
Rotating circular indicator for asynchronous loading.

### When to Use
Use when a section or component is waiting for data of unknown duration.

### When NOT to Use
Do not use when the duration is known and quantifiable; use Progress instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Spin Structural Hierarchy:
├── Root: Spinning circular SVG.
├── Mask: Semi-transparent backdrop when wrapping a container.
├── Tip: Optional loading text below spinner.
```
...
- **Standalone spinner**: Standalone spinner (Small, Default, Large)
- **Container wrap mode**: Container wrap mode (semi-transparent overlay over target content)
...
- **Key Attributes**: `aria-busy="true"`, `aria-label="Loading"`
- **Keyboard Navigation**: Announces active loading state.
