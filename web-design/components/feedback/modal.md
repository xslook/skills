...
> Category: **Feedback**  
> Identifier: `modal`  
> Definition: Centered dialog overlay requiring user attention or confirmation.
...
### Core Purpose
Centered dialog overlay requiring user attention or confirmation.

### When to Use
Use for critical blocking tasks, irreversible confirmations, or self-contained forms.

### When NOT to Use
Do not open a modal from inside another modal. Do not use for simple row deletions; use Popconfirm. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Modal Structural Hierarchy:
├── Mask: Fullscreen dark backdrop (`rgba(0,0,0,0.45)`).
├── Wrap: Centered flex positioning container.
├── Content: Elevated white dialog box.
├── Header: Title.
├── Body: Content slot.
├── Footer: Cancel and Confirm action buttons.
```
...
- **Standard form modal**: Standard form modal (520px)
- **Confirmation dialog**: Confirmation dialog (Modal.confirm)
- **Custom full-screen modal**: Custom full-screen modal
...
- **Key Attributes**: `aria-modal="true"`, `aria-labelledby="modal_title_id"`
- **Keyboard Navigation**: Focus moves to first interactive item; focus trapped inside; Escape dismisses.
