...
> Category: **Feedback**  
> Identifier: `progress`  
> Definition: Linear or circular progress indicator.
...
### When to Use
Use for displaying file upload progress, task completion percentage, or storage quota usage.

### When NOT to Use
Do not use for indefinite loading with unknown duration; use Spin instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Progress Structural Hierarchy:
├── Root: Progress bar container.
├── Track: Inactive background trail.
├── Bar: Active colored fill.
...
- **Line**: Line (horizontal track)
- **Circle**: Circle (radial gauge)
- **Dashboard**: Dashboard (270-degree meter)
...
- **Key Attributes**: `aria-valuenow="75"`, `aria-valuemin="0"`, `aria-valuemax="100"`
- **Keyboard Navigation**: Exposes completion percentage.
