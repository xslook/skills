...
> Definition: Text, headings, paragraphs, and interactive strings with editing and copying.
...
### Core Purpose
Text, headings, paragraphs, and interactive strings with editing and copying.

### When to Use
Use for structured text presentation, collapsible multiline paragraphs, copyable strings, and inline editable text.

### When NOT to Use
Do not use for form inputs; use Input or Textarea instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
├── Root: HTML text node or heading.
├── CopyBtn: Inline copy action button.
├── EditBtn: Inline edit action button.
```
...
- **Title**: Title (H1–H5 headings)
- **Text**: Text (regular, secondary, success, warning, danger, disabled)
- **Paragraph**: Paragraph (multi-line body with ellipsis)
- **Copyable**: Copyable (one-click copy with tooltip)
- **Editable**: Editable (in-place text editing)
...
- **ARIA Role**: `role="text"` / `h1`-`h5` / `p`
- **Key Attributes**: `aria-level="1-5"` on headings
- **Keyboard Navigation**: Editable mode: Enter confirms edit, Escape cancels.
