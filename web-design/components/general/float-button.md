...
> Category: **General**  
> Identifier: `float-button`  
> Definition: Floating action button pinned to the screen for global or page-level shortcuts.
...
### Core Purpose
Floating action button pinned to the screen for global or page-level shortcuts.

### When to Use
Use for quick persistent shortcuts across tall pages (e.g., Back-to-Top, Customer Support, Quick Create).

### When NOT to Use
Do not place in views where it obscures critical table cells or footer action bars. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
FloatButton Structural Hierarchy:
├── Root: Fixed-position circular container with elevated shadow.
├── Icon: Action icon.
├── Badge: Numeric counter badge attached to button.
├── Menu: Expandable sub-button popover list.
```
...
- **Default**: Default (circular floating icon)
- **Primary**: Primary (solid brand fill)
- **Group**: Group (expandable floating menu)
- **BackTop**: BackTop (scrolls to top on click)
...
- **Key Attributes**: `aria-label="Back to top"` or action name
