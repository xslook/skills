...
> Definition: Global configuration abstraction for theme tokens, internationalization, and component prefixing.
...
### Core Purpose
Global configuration abstraction for theme tokens, internationalization, and component prefixing.

### When to Use
Use at application root to inject design tokens, switch between light and dark modes, or set locale strings.

### When NOT to Use
Do not use for styling individual single components directly. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
ConfigProvider Structural Hierarchy:
├── Abstract: Context provider wrapping root tree (no direct DOM output).
```
...
- **Theme override**: Theme override (Token injection)
- **Algorithm override**: Algorithm override (Dark algorithm, Compact algorithm)
...
- **ARIA Role**: Context provider
- **Key Attributes**: None
- **Keyboard Navigation**: No direct DOM output.
