...
> Category: **Layout**  
> Identifier: `flex`  
> Definition: Flexbox layout container providing alignment, direction, and gap control.
...
### Core Purpose
Flexbox layout container providing alignment, direction, and gap control.

### When to Use
Use for 1D horizontal or vertical layouts, alignment controls, and uniform item spacing.

### When NOT to Use
Do not use for complex 2D column-based grid systems; use Grid instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Flex Structural Hierarchy:
├── Root: Flexbox container with customizable justify, align, and gap tokens.
```
...
- **Horizontal**: Horizontal (row flex)
- **Vertical**: Vertical (column flex)
- **Wrap**: Wrap (wrapping flex items)
...
- **Key Attributes**: Semantic tags (`header`, `main`, `section`)
