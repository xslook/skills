...
> Category: **Layout**  
> Identifier: `space`  
> Definition: Uniform spacing container for inline components.
...
### Core Purpose
Uniform spacing container for inline components.

### When to Use
Use to maintain consistent gaps (8px, 12px, 16px) between buttons, tags, or inputs.

### When NOT to Use
Do not use for complex responsive grid layouts; use Grid instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Space Structural Hierarchy:
├── Root: Inline flex container.
├── Item: Wrapper for each child item with gap margins.
```
...
- **Horizontal**: Horizontal (inline items)
- **Vertical**: Vertical (stacked items)
- **Compact**: Compact (fused border group for inputs and buttons)
