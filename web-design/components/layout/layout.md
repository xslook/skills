...
> Category: **Layout**  
> Identifier: `layout`  
> Definition: High-level page skeleton with Header, Sider, Content, and Footer.
...
### Core Purpose
High-level page skeleton with Header, Sider, Content, and Footer.

### When to Use
Use to establish the overarching shell for dashboards, consoles, and web applications.

### When NOT to Use
Do not nest page layouts inside small cards or modal dialogs. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
├── Header: Top banner navigation bar.
├── Sider: Left collapsible sidebar.
├── Content: Fluid main viewport area.
├── Footer: Bottom copyright and links bar.
```
...
- **Left-Right**: Left-Right (fixed sidebar with fluid main body)
- **Top-Bottom**: Top-Bottom (header navigation with centered content)
- **Mixed Layout**: Mixed Layout (top header with nested sidebar)
...
- **ARIA Role**: `role="main"`, `role="banner"`, `role="complementary"`
- **Key Attributes**: W3C Landmark roles
- **Keyboard Navigation**: Standard landmark navigation.
