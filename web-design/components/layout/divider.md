...
> Category: **Layout**  
> Identifier: `divider`  
> Definition: Thin line separating content blocks horizontally or vertically.
...
### Core Purpose
Thin line separating content blocks horizontally or vertically.

### When to Use
Use to divide sections within a card, separate inline actions, or insert section titles along horizontal lines.

### When NOT to Use
Do not use excessive dividers when whitespace alone creates clear separation. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Divider Structural Hierarchy:
├── Root: 1px border line container.
├── InnerContent: Optional title text embedded within the divider.
```
...
- **Horizontal**: Horizontal (full-width section line)
- **Vertical**: Vertical (inline action separator)
- **With Title**: With Title (left, center, or right aligned title text)
...
- **Key Attributes**: `aria-orientation="horizontal|vertical"`
- **Keyboard Navigation**: Non-interactive element.
