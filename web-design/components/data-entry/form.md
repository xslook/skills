...
> Category: **Data-entry**  
> Identifier: `form`  
> Definition: Container for data collection, validation rules, and submission.
...
### Core Purpose
Container for data collection, validation rules, and submission.

### When to Use
Use for any structured data entry workflow requiring validation, error messaging, and submission.

### When NOT to Use
Do not use for instant search query inputs without submit actions. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Form Structural Hierarchy:
├── Root: Form element.
├── Item: Field wrapper.
├── Label: Field title with optional required asterisk.
├── Control: Input element slot.
├── Error: Validation error message slot.
```
...
- **Horizontal**: Horizontal (labels beside inputs)
- **Vertical**: Vertical (labels above inputs)
- **Inline**: Inline (single-row search bar)
...
- **ARIA Role**: `role="form"`
- **Key Attributes**: Invalid fields: `aria-invalid="true"`, `aria-describedby="error_id"`
- **Keyboard Navigation**: Tab traverses fields; Enter submits form. On error, focus moves to first invalid field.
