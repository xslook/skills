...
> Category: **Data-entry**  
> Identifier: `tree-select`  
> Definition: Dropdown selector containing an expandable hierarchical tree.
...
### Core Purpose
Dropdown selector containing an expandable hierarchical tree.

### When to Use
Use when selecting options from an irregular, branching tree structure (e.g., organizational units).

### When NOT to Use
Do not use when tree levels are fixed and uniform; use Cascader instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
TreeSelect Structural Hierarchy:
├── Root: Select control box.
├── Dropdown: Floating tree view container.
├── TreeNode: Expandable node with chevron, checkbox, and label.
```
...
- **Checkable**: Checkable (parent-child linked checkboxes)
...
- **ARIA Role**: `role="combobox"` + `role="tree"`
- **Key Attributes**: `aria-haspopup="tree"`
- **Keyboard Navigation**: Up/Down arrows traverse nodes; Right expands; Left collapses; Enter selects.
