...
> Category: **Data-entry**  
> Identifier: `input`  
> Definition: Basic single-line or multi-line text input field.
...
### Core Purpose
Basic single-line or multi-line text input field.

### When to Use
Use for alphanumeric text entry (usernames, descriptions, search strings).

### When NOT to Use
Do not use for strictly numeric values; use InputNumber instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Input Structural Hierarchy:
├── Root: Input container.
├── Prefix / Suffix: Icon or text badges.
├── Input: HTML text field.
├── ClearBtn: Quick reset icon button.
```
...
- **Outlined**: Outlined (standard)
- **Filled**: Filled (tinted background)
- **Borderless**: Borderless (flush inline text)
- **Password**: Password (visibility toggle)
- **TextArea**: TextArea (multiline with auto-resize)
...
- **ARIA Role**: `role="textbox"`
- **Key Attributes**: `aria-required="true"`, `aria-disabled="true"`
- **Keyboard Navigation**: Standard text entry and clipboard shortcuts. Clear button accepts Tab and Enter.
