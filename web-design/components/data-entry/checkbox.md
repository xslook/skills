...
> Category: **Data-entry**  
> Identifier: `checkbox`  
> Definition: Binary or multi-selection control.
...
### Core Purpose
Binary or multi-selection control.

### When to Use
Use to toggle a single setting within a submitted form, or to select multiple items from a list.

### When NOT to Use
Do not use for instant real-time toggles; use Switch instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Checkbox Structural Hierarchy:
├── Root: Label wrapper.
├── Input: Native checkbox input.
├── Inner: Styled checkbox square with checkmark icon.
├── Text: Accompanying label text.
```
...
- **Indeterminate**: Indeterminate (tri-state partial select)
- **Checkbox Group**: Checkbox Group (horizontal or vertical list)
...
- **Key Attributes**: `aria-checked="true|false|mixed"`
- **Keyboard Navigation**: Tab focuses; Space toggles checked state.
