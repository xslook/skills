...
> Definition: Selects specific times (hours, minutes, seconds).
...
### Core Purpose
Selects specific times (hours, minutes, seconds).

### When to Use
Use for configuring schedules, flight times, or business hours.

### When NOT to Use
Do not use when dates must be selected simultaneously; use DatePicker with time mode. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
TimePicker Structural Hierarchy:
├── Root: Input box with clock icon.
├── Panel: Floating overlay with vertical columns for hours, minutes, and seconds.
```
...
- **12-Hour format**: 12-Hour format (AM/PM)
- **24-Hour format**: 24-Hour format
- **Time RangePicker**: Time RangePicker (start and end time)
...
- **Key Attributes**: `aria-haspopup="dialog"`, `aria-label="Choose time"`
- **Keyboard Navigation**: Up/Down arrows scroll hours/minutes/seconds columns; Left/Right switches column; Enter confirms.
