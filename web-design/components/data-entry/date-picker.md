...
> Definition: Selects dates, times, years, quarters, and date ranges.
...
### Core Purpose
Selects dates, times, years, quarters, and date ranges.

### When to Use
Use whenever a date or date interval must be entered or filtered.

### When NOT to Use
Do not use for pure clock times without dates; use TimePicker instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
DatePicker Structural Hierarchy:
├── Root: Input box with calendar icon.
├── Panel: Floating calendar grid.
├── Header: Month/year switcher.
├── Body: 7-column day grid.
```
...
- **RangePicker**: RangePicker (start and end date)
- **Year / Month / Quarter / Week modes**: Year / Month / Quarter / Week modes
...
- **ARIA Role**: `role="combobox"` + `role="grid"`
- **Key Attributes**: `aria-haspopup="dialog"`, `aria-label="Choose date"`
- **Keyboard Navigation**: Tab focuses input; Enter opens calendar; Arrow keys traverse days; Enter confirms; Escape closes.
