...
> Category: **Data-entry**  
> Identifier: `radio`  
> Definition: Single-select control among mutually exclusive choices.
...
### Core Purpose
Single-select control among mutually exclusive choices.

### When to Use
Use when choosing 1 item from 2 to 6 visible options that need immediate comparison.

### When NOT to Use
Do not use when options exceed 7 items; use Select instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Radio Structural Hierarchy:
├── Root: Label wrapper.
├── Input: Native radio button.
├── Inner: Styled circle with animated center dot.
├── Text: Option label.
```
...
- **Radio button**: Radio button (circular dot)
- **Radio.Group**: Radio.Group (button-tab style)
- **Solid fill vs. Outlined button**: Solid fill vs. Outlined button
...
- **ARIA Role**: `role="radio"`
- **Key Attributes**: Group `role="radiogroup"`, Item `aria-checked="true|false"`
- **Keyboard Navigation**: Tab focuses into radio group; Arrow keys navigate and immediately activate radio item.
