...
> Category: **Data-entry**  
> Identifier: `mentions`  
> Definition: Text field supporting `@` character triggers for mentioning users or tags.
...
### Core Purpose
Text field supporting `@` character triggers for mentioning users or tags.

### When to Use
Use in comments, discussion threads, and collaborative team tools.

### When NOT to Use
Do not use for standard standalone select dropdowns; use Select instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Mentions Structural Hierarchy:
├── Root: Textarea container.
├── Dropdown: Floating mention suggestions list.
├── Item: User or entity suggestion option.
```
...
- **Single-line mention**: Single-line mention
- **Multi-line mention area**: Multi-line mention area
- **Custom prefix trigger**: Custom prefix trigger (`#`, `:`)
...
- **Key Attributes**: `aria-autocomplete="list"`
- **Keyboard Navigation**: Type trigger char (@) to open suggestions; Up/Down arrows traverse; Enter completes.
