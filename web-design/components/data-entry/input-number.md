...
> Definition: Numeric input field with step increment buttons and bounds.
...
### Core Purpose
Numeric input field with step increment buttons and bounds.

### When to Use
Use for quantities, monetary amounts, percentages, and numeric parameters.

### When NOT to Use
Do not use for telephone numbers or credit card numbers (treat those as text with formatting). Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
InputNumber Structural Hierarchy:
├── Root: Input container.
├── Input: Numeric text field.
├── Handler: Vertical stepper buttons (Up/Down).
```
...
- **Standard with up/down stepper buttons**: Standard with up/down stepper buttons
- **Outlined / Filled / Borderless**: Outlined / Filled / Borderless
- **With prefix/suffix units**: With prefix/suffix units
...
- **ARIA Role**: `role="spinbutton"`
- **Key Attributes**: `aria-valuenow`, `aria-valuemin`, `aria-valuemax`
- **Keyboard Navigation**: Up/Down arrows increment/decrement; PageUp/PageDown step by larger increments.
