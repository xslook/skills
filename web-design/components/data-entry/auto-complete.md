...
> Definition: Text input with real-time predictive matching suggestions.
...
### Core Purpose
Text input with real-time predictive matching suggestions.

### When to Use
Use to assist users in typing by suggesting historical inputs or common query completions while still allowing custom free text.

### When NOT to Use
Do not use when users must select strictly from a finite list of options; use Select instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
AutoComplete Structural Hierarchy:
├── Root: Input container.
├── Input: Text field.
├── Popup: Floating suggestion menu list.
├── Item: Matched option item.
```
...
- **Status variants**: Status variants (error, warning)
...
- **Key Attributes**: `aria-autocomplete="list"`, `aria-expanded="true|false"`
- **Keyboard Navigation**: Type to filter; Up/Down arrows traverse suggestions; Enter selects; Escape dismisses.
