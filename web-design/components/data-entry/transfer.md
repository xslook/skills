...
> Category: **Data-entry**  
> Identifier: `transfer`  
> Definition: Double-column transfer control for moving items between two sets.
...
### Core Purpose
Double-column transfer control for moving items between two sets.

### When to Use
Use for assigning permissions, allocating roles to members, or configuring column lists.

### When NOT to Use
Do not use for simple single-list selections; use Select (multiple) instead. Consult the [Component Decision Tree](../../references/component-decision-tree.md) for alternatives.
...
```text
Transfer Structural Hierarchy:
├── SourceList: Left item column.
├── Operations: Center action buttons (`>` and `<`).
├── TargetList: Right selected column.
```
...
- **Standard dual-column**: Standard dual-column
- **With search filter bars**: With search filter bars
- **Custom tree or table transfer**: Custom tree or table transfer
...
- **Key Attributes**: `aria-label="Transfer list selector"`
- **Keyboard Navigation**: Tab cycles between source list, action buttons, and target list; Space checks item; Enter transfers.
